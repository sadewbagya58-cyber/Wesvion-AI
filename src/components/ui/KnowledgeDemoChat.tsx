  ...
  const handleSend = async (userText: string) => {
    ...
    try {
      ...
      const aiMsg: DemoMessage = {
        id: createId("ai"),
        sender: "ai",
        text: data.answer || "I couldn't find that information in the uploaded hotel documents.",
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      ...
      const fallbackMsg: DemoMessage = {
        id: createId("ai"),
        sender: "ai",
        text: "I couldn't find that information in the uploaded hotel documents.",
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
        sources: [],
      };
      ...
      <div className="p-6 h-[460px] overflow-y-auto space-y-4 bg-stone-900/40">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-8 text-stone-500 space-y-3">
            ...
            <p className="font-serif text-stone-300 text-base">Grounded Knowledge Assistant (v2)</p>
            <p className="text-xs text-stone-400 max-w-md">
              Ask any question about Aura Boutique Hotel&apos;s dining menu, spa rituals, or weekly events. Answers are generated by AI strictly using uploaded property documents.
            </p>
          </div>
        ) : (
          messages.map((msg) => (
            ...
            >
              ...
              <div className={`max-w-[78%] space-y-1.5 ${msg.sender === "guest" ? "text-right" : "text-left"}`}>
                ...
                {/* Source Badges ONLY (No raw chunk text) */}
                {msg.sender === "ai" && msg.sources && msg.sources.length > 0 && (
                  <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                    {msg.sources.map((src, sIdx) => (
                      <span
                        key={sIdx}
                        className="inline-flex items-center gap-1 bg-amber-950/50 text-amber-300/90 text-[11px] px-2.5 py-0.5 rounded-full border border-amber-800/40"
                      >
                        <FileText className="w-3 h-3 text-amber-400 shrink-0" />
                        <span className="truncate max-w-[240px]">{src.documentTitle}</span>
                      </span>
                    ))}
                  </div>
                )}
        ...
        {isTyping && (
          <div className="flex items-center gap-3">
            ...
            <div className="bg-stone-800/90 border border-stone-700/50 px-4 py-3 rounded-2xl rounded-tl-none flex items-center gap-1.5 text-xs text-stone-400">
              ...
              <span className="ml-1 text-stone-400 font-serif">Generating grounded AI answer...</span>
            </div>
          </div>
        )}
      </div>
